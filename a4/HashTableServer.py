...
from HashTableUtil import HTFileMetadata, HTMessage, HTMessageBody, HashTableFile
    ...
    def handle_request(self,conn):
            ...
            try:
                    ...
                    if message.intent=="INSERT":
                        try:
                            metadata = HTFileMetadata(filename=message.body.filename, 
                                                      size=message.body.file.size, 
                                                      file=message.body.file, 
                                                      path_on_disk='ht_server_file' + str(uuid.uuid4()))
                            
                            metadata.file.to_disk(metadata.path_on_disk)
                            metadata.file = None  # free memory

                            self.table.insert(message.body.filename,metadata)
                        ...
                        try:
                            ...
                            # this should only persist until sent?
                            file = HashTableFile(result.filename).from_disk(result.path_on_disk)
                            response = HTMessage("STATUS", HTMessageBody(status="OK", file=file))
                        except KeyError:
                            response = HTMessage("STATUS", HTMessageBody(status="KEYERROR"))
